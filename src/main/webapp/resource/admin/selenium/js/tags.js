	...
	$.ajax({
		...
		success : function(result) {
			...
			$.each(tags, function(index, item) {
				...
				var tagId = $("<td></td>").append(item.tagId);
				var tagName = $("<td></td>").append(item.tagName);
				var tagDescription = $("<td></td>").append(
						item.tagDescription);
				var buttons = $("<td></td>");
				...
				tr.append(checkbox).append(tagId).append(tagName)
						.append(tagDescription)
						.append(buttons).appendTo($("#tags"));
				build_page_info(result);
				build_page_line(result);
				// 批量选择的选择框全选或者不全选
				selectCheckBox();
				// 为按钮绑定事件
				$("#btn-update-" + item.tagId).click(function() {
					build_update_modal(item.tagId);
				});
				$("#btn-delete-" + item.tagId).click(function() {
					var flag = confirm("是否删除标签'" + item.tagName + "'?");
					if (flag == true) {
...
function update(button) {// 修改标签
	$.ajax({
		...
		type : "POST",
		data : $("#updateForm").serialize() + "&tagId="
